# 📚 Kori UI - Feature Documentation

Complete guide to all features and modules in Kori UI.
...
### Configuration Path
`/kori` → **Unit Frames** section
`/kori editmode` for positioning

### Technical Details
- **Module**: `kori_unitframes.lua`
...
### Configuration Path
`/kori` → **Castbars** section

### Technical Details
- **Module**: `kori_castbar.lua`
...
### Configuration Path
`/kori` → **Minimap** section

### Technical Details
- **Module**: `koricore_minimap.lua`
...
### Configuration Path
`/kori` → **Buff Bars** section

### Technical Details
- **Module**: `kori_buffbar.lua`
...
1. **Enable CDM**: `/kori` → **Cooldown Manager** → Enable
2. **Enter Edit Mode**: `/kori editmode` or standard Edit Mode
3. **Position Bars**: Drag CDM bars to desired locations
4. **Adjust Size**: Use Edit Mode scaling or `/kori` settings
...
### Configuration Path
`/kori` → **Cooldown Manager** section
`/cdm` for quick settings access

### Technical Details
- **Module**: `kori_ncdm.lua`
...
- **Integration**: Access via `/kori` → **M+ & Raiding** → **Teleports**
...
### Configuration Path
`/kori` → **M+ & Raiding** section

### Technical Details
- **M+ Timer Module**: `kori_mplus_timer.lua`
- **Keystone Module**: `kori_keystone.lua`
- **Dungeon Data**: `kori_dungeon_data.lua` (season-aware)
...
### Configuration Path
`/kori` → **Combat Enhancements** section

### Technical Details
- **Combat Text Module**: `kori_combattext.lua`
- **Combat Timer Module**: `kori_combattimer.lua`
- **Rotation Assist Module**: `kori_rotationassist.lua`
...
1. **Open GUI**: `/kori` → **Custom Trackers**
...
#### Shadow Priest: Void form Tracker
...
### Configuration Path
`/kori` → **Custom Trackers** section

### Technical Details
- **Module**: `kori_customtrackers.lua`
...
### Configuration Path
`/kori` → **Tooltips** section

### Technical Details
- **Module**: `kori_tooltips.lua`
...
#### Quick Access
- **Game Menu Shortcuts**: Adds shortcuts to /kori in game menu
...
### Configuration Path
`/kori` → **Quality of Life** section

### Technical Details
- **Quick Salvage Module**: `kori_quicksalvage.lua`
- **QoL Module**: `kori_qol.lua`
- **Skyriding Module**: `kori_skyriding.lua`
...
### Configuration Path
`/kori` → **Anchoring** section

### Technical Details
- **Module**: `kori_anchoring.lua` + `kori_anchoring_options.lua`
...
#### How to Use
1. **Open GUI**: `/kori` → **Performance** tab
2. **View Current Settings**: See your current CVar values
3. **Apply Preset**: Click "Apply Kori FPS Preset"
...
#### Garbage Collection
- **Manual GC**: Force garbage collection via `/kori` → **Performance**
...
### Configuration Path
`/kori` → **Performance** section

### Technical Details
- **FPS Module**: Embedded in `kori_options.lua`
...
- Timer display for readycheck expiry
...
- Check `/kori` → **Actionbars** → ensure bars are not set to fade
...
#### Performance Issues
- Apply FPS preset via `/kori` → **Performance**
- Disable unused modules
- Reduce number of custom trackers
- Check for conflicting addons

#### Text/Fonts Appearing Blurry
- Use Perfect Pixel via `/kori` → **Performance** → **Apply Perfect Pixel**
...
If issues persist:
1. Check [Known Issues](https://github.com/korivash/KoriUI/issues)
2. Disable other addons to test for conflicts
3. Report bugs with steps to reproduce
4. Include WoW version, Kori UI version, and error messages (if any)
...
Kori UI uses a modular architecture:
- **Core**: `koricore_main.lua` - Main initialization and profile management
...
-- Hook into KORICore initialization
KORICore:RegisterEvent("ADDON_LOADED")
KORICore.InitMyModule = InitModule
```

### API Documentation

#### KORICore Global Functions

- `KORICore:SafeReload()` - Safely reload UI (queues if in combat)
- `KORICore:GetDB()` - Returns AceDB database object
- `KORICore:RegisterOption(name, table)` - Register options for GUI
- `KORICore:GetTexture(name)` - Get texture from LibSharedMedia
- `KORICore:GetFont(name)` - Get font from LibSharedMedia

#### Creating Custom Trackers Programmatically

```lua
local trackerId = KORICore:CreateCustomTracker({
    name = "My Tracker",
    spellId = 12345,
    triggerType = "buff",
    unitType = "player",
