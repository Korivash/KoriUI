...
function KoriUI:BackwardsCompat()
    ...
    if KoriUI_DB and KoriUI_DB.profiles and KoriUI_DB.profiles.Default then
        self:DebugPrint("Profiles.Default.imports Exists: " .. tostring(not (not KoriUI_DB.profiles.Default.imports)))
        ...
        if KoriUI_DB.profiles.Default.imports and (not self.db.global.imports or next(self.db.global.imports) == nil) then
            self:DebugPrint("Import Data found in profile imports but not global imports.")
            self.db.global.imports = KoriUI_DB.profiles.Default.imports
        end
    end
end
