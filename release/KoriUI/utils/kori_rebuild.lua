-- KoriUI Rebuild Layer
-- Replaces legacy options shell with an original "Command Deck" layout.

local KORI = KoriUI
if not KORI or not KORI.GUI then return end
...
C.bg = {0.03, 0.05, 0.08, 0.98}
C.bgLight = {0.06, 0.09, 0.14, 1}
C.bgDark = {0.01, 0.02, 0.04, 1}
C.bgContent = {0.04, 0.07, 0.11, 0.88}

C.accent = {0.20, 0.76, 0.94, 1}
C.accentLight = {0.52, 0.88, 1.00, 1}
C.accentDark = {0.09, 0.47, 0.62, 1}
C.accentHover = {0.66, 0.93, 1.00, 1}

C.tabSelected = {0.20, 0.76, 0.94, 1}
C.tabSelectedText = {0.94, 0.99, 1, 1}
C.tabNormal = {0.68, 0.77, 0.86, 1}
C.tabHover = {1, 1, 1, 1}

C.text = {0.87, 0.92, 0.97, 1}
C.textBright = {1, 1, 1, 1}
C.textMuted = {0.58, 0.67, 0.75, 1}

C.border = {0.10, 0.20, 0.29, 1}
C.borderLight = {0.20, 0.37, 0.50, 1}
C.borderAccent = {0.20, 0.76, 0.94, 1}
C.sectionHeader = {0.55, 0.89, 1.00, 1}

C.navBg = {0.02, 0.05, 0.08, 1}
C.navItemBg = {0.06, 0.11, 0.16, 1}
C.navItemActiveBg = {0.08, 0.20, 0.29, 1}
C.navActionBg = {0.08, 0.15, 0.21, 1}
...
function GUI:CreateMainFrame()
    ...
    local savedWidth = (KORI.KORICore and KORI.KORICore.db and KORI.KORICore.db.profile and KORI.KORICore.db.profile.configPanelWidth) or defaultWidth
    local savedHeight = (KORI.KORICore and KORI.KORICore.db and KORI.KORICore.db.profile and KORI.KORICore.db.profile.configPanelHeight) or defaultHeight

    local frame = CreateFrame("Frame", "KoriUI_CommandDeck", UIParent, "BackdropTemplate")
    ...
    local version = KORI.versionString or "dev"
    ...
    local tag = header:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    tag:SetPoint("TOPRIGHT", -18, -16)
    UseTitleFont(tag, 11, C.textMuted)
    tag:SetText("v" .. version)
    ...
    frame:SetScript("OnSizeChanged", function(self, width, height)
        ...
        if KORI.KORICore and KORI.KORICore.db and KORI.KORICore.db.profile then
            KORI.KORICore.db.profile.configPanelWidth = math.floor(width + 0.5)
            KORI.KORICore.db.profile.configPanelHeight = math.floor(height + 0.5)
        end
    end)
