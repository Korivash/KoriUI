...
local function MakeURLsClickable(text)
    ...
    local success, result = pcall(function()
        ...
        local linkFormat = "|cff" .. colorHex .. "|Haddon:koriuichat:%1|h[%1]|h|r"
...
local function CreateCopyPopup()
    ...
    urlPopup = CreateFrame("Frame", "KoriUI_ChatCopyPopup", UIParent, "BackdropTemplate")
    ...
    title:SetTextColor(KORI_COLORS.accent[1], KORI_COLORS.accent[2], KORI_COLORS.accent[3], 1)
    ...
    editBox:SetTextColor(KORI_COLORS.text[1], KORI_COLORS.text[2], KORI_COLORS.text[3], 1)
    ...
    if not tContains(UISpecialFrames, "KoriUI_ChatCopyPopup") then
        tinsert(UISpecialFrames, "KoriUI_ChatCopyPopup")
    end
...
local function SetupURLClickHandler()
    ...
    EventRegistry:RegisterCallback("SetItemRef", function(_, link, text, button)
        ...
        local url = link:match("^addon:koriuichat:(.*)")
        if url then
            ShowCopyPopup(url)
            return true
        end
...
local function CreateChatCopyFrame()
    ...
    chatCopyFrame = CreateFrame("Frame", "KoriUI_ChatCopyFrame", UIParent, "BackdropTemplate")
    ...
    chatCopyFrame:SetBackdropColor(KORI_COLORS.bg[1], KORI_COLORS.bg[2], KORI_COLORS.bg[3], KORI_COLORS.bg[4])
    chatCopyFrame:SetBackdropBorderColor(KORI_COLORS.accent[1], KORI_COLORS.accent[2], KORI_COLORS.accent[3], KORI_COLORS.accent[4])
    ...
    -- Add to special frames so ESC closes it
    if not tContains(UISpecialFrames, "KoriUI_ChatCopyFrame") then
        tinsert(UISpecialFrames, "KoriUI_ChatCopyFrame")
    end
...
local function GetOrCreateCopyButton(chatFrame)
    ...
    local button = CreateFrame("Button", frameName .. "KorivashCopyButton", chatFrame)
...
local function SetupCopyButtonHoverMode(chatFrame)
    ...
    if chatFrame.koriCopyButtonHooked then return end
    chatFrame.koriCopyButtonHooked = true
...
local function ApplyCopyButtonMode(chatFrame)
    ...
    elseif mode == "hover" then
        ...
        if not chatFrame.koriCopyButtonHooked then
...
local function UpdateTabColors(tab)
    ...
    if isSelected then
        ...
        tab.__quiBackdrop:SetBackdropBorderColor(KORI_COLORS.accent[1], KORI_COLORS.accent[2], KORI_COLORS.accent[3], 1)
    else
