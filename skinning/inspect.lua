...
local function CreateOrUpdateBackground()
    ...
    if not customBg then
        customBg = CreateFrame("Frame", "KORI_InspectFrameBg_Skin", InspectFrame, "BackdropTemplate")
...
---------------------------------------------------------------------------
-- Main skinning setup
-- Note: OnShow hook is handled by kori_character.lua which calls SetExtended()
...
local function SetupInspectFrameSkinning()
    ...
    -- Create initial background (will be positioned by kori_character.lua via SetExtended)
    CreateOrUpdateBackground()
...
_G.KoriUI_RefreshInspectColors = RefreshInspectFrameColors
