...
---------------------------------------------------------------------------
local KORI_Castbar = {}
ns.KORI_Castbar = KORI_Castbar

KORI_Castbar.castbars = {}
...
-- SETUP HELPERS
---------------------------------------------------------------------------
function KORI_Castbar:SetHelpers(helpers)
    Helpers = helpers or {}
end
...
KORI_Castbar.STAGE_FILL_COLORS = {
...
local STAGE_COLORS = KORI_Castbar.STAGE_COLORS
local STAGE_FILL_COLORS = KORI_Castbar.STAGE_FILL_COLORS
...
function KORI_Castbar:GetEmpoweredLevel()
    local playerCastbar = self.castbars["player"]
    if not playerCastbar then
        playerCastbar = _G.KoriUI_Castbars and _G.KoriUI_Castbars["player"]
    end
...
---------------------------------------------------------------------------
function KORI_Castbar:CreateCastbar(unitFrame, unit, unitKey)
    local settings = GetUnitSettings(unitKey)
...
---------------------------------------------------------------------------
function KORI_Castbar:SetupCastbar(castbar, unit, unitKey, castSettings)
        ...
        if spellName or channelName or isInEmpoweredHold then
            ...
            if isPlayer and self.isEmpowered then
                ...
                if self.empoweredLevelText and self.showEmpoweredLevel then
                    local currentStage, maxStages, isEmpowered = KORI_Castbar:GetEmpoweredLevel()
...
function KORI_Castbar:SetupTargetFocusCastbar(castbar, unit, unitKey, castSettings)
    self:SetupCastbar(castbar, unit, unitKey, castSettings)
end

function KORI_Castbar:SetupPlayerCastbar(castbar, unit, unitKey, castSettings)
    self:SetupCastbar(castbar, unit, unitKey, castSettings)
end
...
-- BOSS CASTBAR SETUP
---------------------------------------------------------------------------
function KORI_Castbar:SetupBossCastbar(castbar, unit, bossIndex, castSettings)
    -- Unified OnUpdate handler - handles both real casts and preview
    local function CastBar_OnUpdate(self, elapsed)
        -- Check if actually casting (real cast takes priority)
        local spellName = UnitCastingInfo(self.unit)
        ...
        if spellName and startTimeMS and endTimeMS then
            ...
            C_Timer.After(0.1, function()
                if not UnitCastingInfo(self.unit) and not UnitChannelInfo(self.unit) then
                    ...
                    local KORI_UF = KORI_Castbar.unitFramesModule
                    local bossFramePreviewActive = KORI_UF and KORI_UF.previewMode and KORI_UF.previewMode["boss" .. bossIndex]
...
-- CREATE: Boss Castbar
---------------------------------------------------------------------------
function KORI_Castbar:CreateBossCastbar(unitFrame, unit, bossIndex)
    ...
    local anchorFrame = CreateAnchorFrame("KORI_Boss" .. bossIndex .. "_Castbar", UIParent)
    anchorFrame:SetSize(castWidth, barHeight)
        ...
        if spellName and startTimeMS and endTimeMS then
            ...
            C_Timer.After(0.1, function()
                if not UnitCastingInfo(self.unit) and not UnitChannelInfo(self.unit) then
                    local settings = GetUnitSettings(self.unitKey)
                    local KORI_UF = KORI_Castbar.unitFramesModule
                    local bossFramePreviewActive = KORI_UF and KORI_UF.previewMode and KORI_UF.previewMode["boss" .. self.bossIndex]
                    if settings and settings.castbar and settings.castbar.previewMode and bossFramePreviewActive then
                        -- Show preview simulation
...
-- GLOBAL FUNCTIONS
---------------------------------------------------------------------------
KORI_Castbar.unitFramesModule = nil

function KORI_Castbar:SetUnitFramesModule(ufModule)
...
_G.KoriUI_ShowCastbarPreview = function(unitKey)
    local settings = GetUnitSettings(unitKey)
    ...
    local KORI_UF = KORI_Castbar.unitFramesModule
    if KORI_UF then
        KORI_UF:RefreshFrame(unitKey)
    end
end

_G.KoriUI_HideCastbarPreview = function(unitKey)
    local settings = GetUnitSettings(unitKey)
    if not settings or not settings.castbar then
        return
    end
...
---------------------------------------------------------------------------
function KORI_Castbar:RefreshCastbar(castbar, unitKey, castSettings, unitFrame)
    ...
    if newCastbar then
        local KORI_UF = self.unitFramesModule
        if KORI_UF and KORI_UF.castbars then
            KORI_UF.castbars[unitKey] = newCastbar
        end
    end
end

function KORI_Castbar:RefreshBossCastbar(castbar, bossKey, castSettings, unitFrame)
    if not castSettings or not unitFrame then return end
    ...
    if newCastbar then
        local KORI_UF = self.unitFramesModule
        if KORI_UF and KORI_UF.castbars then
            KORI_UF.castbars[bossKey] = newCastbar
        end
    end
end

_G.KoriUI_RefreshCastbar = function(unitKey)
    local KORI_UF = KORI_Castbar.unitFramesModule
    if not KORI_UF then return end
    KORI_UF:RefreshFrame(unitKey)
end

-- Refresh all castbars (used by HUD Layering options)
_G.KoriUI_RefreshCastbars = function()
    local KORI_UF = KORI_Castbar.unitFramesModule
    if not KORI_UF then return end
...
_G.KoriUI_Castbars = KORI_Castbar.castbars
