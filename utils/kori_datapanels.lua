...
function Datapanels:CreatePanel(panelID, config)
    if self.activePanels[panelID] then
        print("|cffff0000KoriUI:|r Panel '" .. panelID .. "' already exists!")
        return self.activePanels[panelID]
    end
    
    -- Create panel frame
    local panel = CreateFrame("Frame", "KORI_Datapanel_" .. panelID, UIParent)
...
SlashCmdList["QUIDATAPANELS"] = function(msg)
    if msg == "show" then
        ...
        print(string.format("|cff00ff00KoriUI:|r Total panels: %d", count))
