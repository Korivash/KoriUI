...
local function HideCooldownEffects(child)
    ...
    for _, frameName in ipairs(effectFrames) do
        ...
        if frame then
            ...
            if not frame._KoriUI_NoShow then
                frame._KoriUI_NoShow = true
...
local function ProcessViewer(viewerName)
        ...
        for _, child in ipairs(children) do
            if child:IsShown() then
                ...
                -- Mark as processed (no OnUpdate hook needed - we handle glows via hooksecurefunc)
                    child._KoriUI_EffectsHidden = true
            end
        end
    end
    ...
    if viewer.Layout and not viewer._KoriUI_EffectsHooked then
        viewer._KoriUI_EffectsHooked = true
    ...
    -- Hook OnShow
    if not viewer._KoriUI_EffectsShowHooked then
        viewer._KoriUI_EffectsShowHooked = true
...
_G.KoriUI_RefreshCooldownEffects = function()
    ApplyToAllViewers()
end
