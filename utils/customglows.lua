...
_G.KoriUI_RefreshCustomGlows = RefreshAllGlows
    ...
    if viewer then
        ...
        for i, child in ipairs(children) do
            if child:IsShown() then
                StartGlow(child)
                print("|cFF00FF00[KoriUI]|r Test glow applied to " .. viewerType .. " icon #" .. i)
                return
            end
        end
        print("|cFFFF0000[KoriUI]|r No visible icons in " .. viewerType .. " viewer")
    else
        print("|cFFFF0000[KoriUI]|r " .. viewerType .. "CooldownViewer not found")
    end
end

_G.KoriUI_StopAllCustomGlows = function()
    for icon, _ in pairs(activeGlowIcons) do
        if icon then
            StopGlow(icon)
        end
    ...
    wipe(activeGlowIcons)
    print("|cFF00FF00[KoriUI]|r All custom glows stopped")
end
