...
combatEndFrame:SetScript("OnEvent", function()
    ...
    C_Timer.After(0.1, function()
        if not InCombatLockdown() and _G.KoriUI_RefreshNCDM then
...
local function LayoutViewer(viewerName, trackerKey)
    ...
    if not viewer.__cdmUpdatePending then
        ...
        C_Timer.After(0.05, function()
            ...
            if trackerKey == "essential" then
                if _G.KoriUI_UpdateLockedPowerBar then
                    _G.KoriUI_UpdateLockedPowerBar()
                end
                if _G.KoriUI_UpdateLockedSecondaryPowerBar then
                    _G.KoriUI_UpdateLockedSecondaryPowerBar()
                end
                if _G.KoriUI_UpdateLockedCastbarToEssential then
                    _G.KoriUI_UpdateLockedCastbarToEssential()
                end
            elseif trackerKey == "utility" then
                if _G.KoriUI_UpdateLockedPowerBarToUtility then
                    _G.KoriUI_UpdateLockedPowerBarToUtility()
                end
                if _G.KoriUI_UpdateLockedSecondaryPowerBarToUtility then
                    _G.KoriUI_UpdateLockedSecondaryPowerBarToUtility()
                end
                if _G.KoriUI_UpdateLockedCastbarToUtility then
                    _G.KoriUI_UpdateLockedCastbarToUtility()
                end
            end
            -- Unit frames anchored to CDM (can be anchored to either Essential or Utility)
            if _G.KoriUI_UpdateCDMAnchoredUnitFrames then
                _G.KoriUI_UpdateCDMAnchoredUnitFrames()
            end
            -- Update keybind text on CDM icons
            if _G.KoriUI_UpdateViewerKeybinds then
                _G.KoriUI_UpdateViewerKeybinds(viewerName)
            end
        end)
    end
end
    ...
    viewer:HookScript("OnShow", function(self)
        ...
        C_Timer.After(0.02, function()
            if self:IsShown() then
                ...
                if trackerKey == "utility" and _G.KoriUI_ApplyUtilityAnchor then
                    _G.KoriUI_ApplyUtilityAnchor()
                end
            end
        end)
...
_G.KoriUI_RefreshNCDM = RefreshAll
_G.KoriUI_IncrementNCDMVersion = IncrementSettingsVersion
_G.KoriUI_ApplyUtilityAnchor = ApplyUtilityAnchor
...
local function GetUnitframeFrames()
    ...
    -- Collect unit frames
    if _G.KoriUI_UnitFrames then
        for unitKey, frame in pairs(_G.KoriUI_UnitFrames) do
            if frame then
    ...
    if not (vis and vis.alwaysShowCastbars) then
        if _G.KoriUI_Castbars then
            for unitKey, castbar in pairs(_G.KoriUI_Castbars) do
                if castbar then
...
local function UpdateUnitframesVisibility()
    ...
    if _G.KoriUI_Castbars then
        ...
        if vis and vis.alwaysShowCastbars then
            ...
            if _G.KoriUI_UnitFrames then
                for _, frame in pairs(_G.KoriUI_UnitFrames) do
                    if frame then
...
_G.KoriUI_RefreshCDMVisibility = UpdateCDMVisibility
_G.KoriUI_RefreshUnitframesVisibility = UpdateUnitframesVisibility
_G.KoriUI_RefreshCDMMouseover = SetupCDMMouseoverDetector
_G.KoriUI_RefreshUnitframesMouseover = SetupUnitframesMouseoverDetector
