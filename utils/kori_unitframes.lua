...
---------------------------------------------------------------------------
local KORI_UF = {}
ns.KORI_UnitFrames = KORI_UF

-- Frame references
KORI_UF.frames = {}
KORI_UF.castbars = {}
KORI_UF.previewMode = {}
KORI_UF.auraPreviewMode = {}  -- Tracks buff/debuff preview state (keyed by "unitKey_buff" or "unitKey_debuff")

-- Reference to castbar module
local KORI_Castbar = ns.KORI_Castbar

-- Edit Mode: Slider registry for real-time sync during drag
-- Format: { unitKey = { x = sliderRef, y = sliderRef, coordText = fontStringRef } }
KORI_UF.editModeSliders = {}
...
local function GetTexturePath(textureName)
    ...
    if not name or name == "" then
        local general = GetGeneralSettings()
        name = general and general.texture or "Korivash"
    end
...
local function CreateBossFrame(unit, frameKey, bossIndex)
    ...
    -- Create secure button with unique name (KORI_Boss1, KORI_Boss2, etc.)
    local frameName = "KORI_Boss" .. bossIndex
    ...
    frame:HookScript("OnShow", function(self)
        ...
        if KORI_UF.previewMode[bossKey] then return end
        UpdateFrame(self)
    end)
...
local function ForceUpdateToT()
    local totFrame = KORI_UF.frames and KORI_UF.frames.targettarget
...
local function CreateUnitFrame(unit, unitKey)
    ...
    local frameName = "KORI_" .. unitKey:gsub("^%l", string.upper)
    local frame = CreateFrame("Button", frameName, UIParent, "SecureUnitButtonTemplate, BackdropTemplate, PingableUnitFrameTemplate")
...
local function CreateCastbar(unitFrame, unit, unitKey)
    ...
    if KORI_Castbar then
        return KORI_Castbar:CreateCastbar(unitFrame, unit, unitKey)
    end
    return nil
end
...
local function CreateBossCastbar(unitFrame, unit, bossIndex)
    -- Delegate to castbar module
    if KORI_Castbar then
        return KORI_Castbar:CreateBossCastbar(unitFrame, unit, bossIndex)
    end
    return nil
end
...
local function UpdateAuras(frame)
    ...
    local buffPreviewActive = KORI_UF.auraPreviewMode[unitKey .. "_buff"]
    local debuffPreviewActive = KORI_UF.auraPreviewMode[unitKey .. "_debuff"]
...
---------------------------------------------------------------------------
function KORI_UF:ShowPreview(unitKey)
        ...
        for i = 1, 5 do
            ...
            if frame then
                ...
                if settings and settings.castbar and settings.castbar.previewMode then
                    ...
                    if castbar and KORI_Castbar then
                        -- Trigger the castbar to show its preview
                        KORI_Castbar:RefreshBossCastbar(castbar, bossKey, settings.castbar, frame)
                    end
                end
...
function KORI_UF:HidePreview(unitKey)
...
---------------------------------------------------------------------------
function KORI_UF:ShowAuraPreview(unitKey, auraType)
...
function KORI_UF:ShowAuraPreviewForFrame(frame, unitKey, auraType)
    if not frame then return end
...
function KORI_UF:HideAuraPreview(unitKey, auraType)
...
function KORI_UF:HideAuraPreviewForFrame(frame, unitKey, auraType)
    if not frame then return end
...
---------------------------------------------------------------------------
function KORI_UF:RefreshFrame(unitKey)
    ...
    if unitKey == "boss" then
        ...
        for i = 1, 5 do
            ...
            if frame then
                ...
                local castbar = self.castbars[bossKey]
                if castbar and KORI_Castbar and KORI_Castbar.RefreshBossCastbar then
                    local castSettings = settings.castbar
                    if castSettings then
                        KORI_Castbar:RefreshBossCastbar(castbar, bossKey, castSettings, frame)
                    end
                end
            end
        end
    ...
    if castSettings and castSettings.enabled then
        if castbar and KORI_Castbar and KORI_Castbar.RefreshCastbar then
            -- Castbar exists - refresh it
            KORI_Castbar:RefreshCastbar(castbar, unitKey, castSettings, frame)
        elseif not castbar and KORI_Castbar and KORI_Castbar.CreateCastbar then
            -- Castbar doesn't exist but is now enabled - create it
            self.castbars[unitKey] = KORI_Castbar:CreateCastbar(frame, unitKey, unitKey)
        end
    end
end

function KORI_UF:RefreshAll()
...
---------------------------------------------------------------------------
KORI_UF.editModeActive = false

-- Register slider references for real-time sync during edit mode
function KORI_UF:RegisterEditModeSliders(unitKey, xSlider, ySlider)
...
function KORI_UF:NotifyPositionChanged(unitKey, offsetX, offsetY)
...
local function CreateNudgeButton(parent, direction, deltaX, deltaY, unitKey)
    ...
    btn:SetScript("OnClick", function()
        ...
        if settings then
            ...
            KORI_UF:NotifyPositionChanged(settingsKey, settings.offsetX, settings.offsetY)
        end
    end)
...
function KORI_UF:EnableEditMode()
    ...
    if not self.exitEditModeBtn then
        local exitBtn = CreateFrame("Button", "KORI_ExitEditModeBtn", UIParent, "BackdropTemplate")
        ...
        exitBtn:SetScript("OnClick", function()
            if EditModeManagerFrame and EditModeManagerFrame:IsShown() then
                ...
                -- Fallback for /kori editmode case
                KORI_UF:DisableEditMode()
            end
        end)
    ...
    for unitKey, frame in pairs(self.frames) do
        ...
        frame:SetScript("OnDragStart", function(self)
            if KORI_UF.editModeActive then
                ...
                self:SetScript("OnUpdate", function(self)
                    ...
                    if selfX and selfY and parentX and parentY then
                        ...
                        if settings then
                            ...
                            KORI_UF:NotifyPositionChanged(settingsKey, offsetX, offsetY)
                        end
                    end
                end)
            end
        ...
        frame:SetScript("OnKeyDown", function(self, key)
            if not KORI_UF.editModeActive then return end
...
function KORI_UF:HideBlizzardFrames()
    local db = GetDB()
...
function KORI_UF:Initialize()
    ...
    if KORI_Castbar then
        ...
        KORI_Castbar:SetUnitFramesModule(self)
        KORI_Castbar.castbars = self.castbars
    end
...
function KORI_UF:HookBlizzardEditMode()
    ...
    -- Track if we triggered from Blizzard Edit Mode (vs /kori editmode)
    self.triggeredByBlizzEditMode = false

    hooksecurefunc(EditModeManagerFrame, "EnterEditMode", function()
        if InCombatLockdown() then return end
        if self.editModeActive then return end  -- Already active via /kori editmode
    ...
    hooksecurefunc(EditModeManagerFrame, "ExitEditMode", function()
        ...
        if not self.triggeredByBlizzEditMode then return end  -- Don't exit if user used /kori editmode
        self.triggeredByBlizzEditMode = false
        self:DisableEditMode()
    end)
end
...
---------------------------------------------------------------------------
function KORI_UF:RegisterWithClique()
...
initFrame:SetScript("OnEvent", function(self, event)
    if event == "PLAYER_LOGIN" then
        ...
        C_Timer.After(0.5, function()
            ...
            KORI_UF:HookBlizzardEditMode()
            -- Register frames with Clique after creation
            C_Timer.After(0.5, function()
                KORI_UF:RegisterWithClique()
            end)
        end)
    ...
    elseif event == "PLAYER_REGEN_ENABLED" then
        ...
        if KORI_UF.pendingInitialize then
            KORI_UF.pendingInitialize = false
            KORI_UF:Initialize()
            KORI_UF:HookBlizzardEditMode()
            C_Timer.After(0.5, function()
                KORI_UF:RegisterWithClique()
...
_G.KoriUI_RefreshAuras = function(unitKey)
    if unitKey then
        ...
        if unitKey == "boss" then
            ...
            local frame = KORI_UF.frames[unitKey]
            if frame then
                lastAuraUpdate[unitKey] = 0
                UpdateAuras(frame)
            end
        ...
        for _, key in ipairs({"player", "target", "focus", "pet", "targettarget"}) do
            local frame = KORI_UF.frames[key]
            if frame then
...
_G.KoriUI_ShowUnitFramePreview = function(unitKey)
    KORI_UF:ShowPreview(unitKey)
end

_G.KoriUI_HideUnitFramePreview = function(unitKey)
    KORI_UF:HidePreview(unitKey)
end

_G.KoriUI_ShowAuraPreview = function(unitKey, auraType)
    KORI_UF:ShowAuraPreview(unitKey, auraType)
end

_G.KoriUI_HideAuraPreview = function(unitKey, auraType)
    KORI_UF:HideAuraPreview(unitKey, auraType)
end

_G.KoriUI_ToggleUnitFrameEditMode = function()
    KORI_UF:ToggleEditMode()
end

-- Register slider references for real-time sync during edit mode
_G.KoriUI_RegisterEditModeSliders = function(unitKey, xSlider, ySlider)
    KORI_UF:RegisterEditModeSliders(unitKey, xSlider, ySlider)
end

-- Global references for external access
_G.KoriUI_UnitFrames = KORI_UF.frames
_G.KoriUI_Castbars = KORI_UF.castbars

-- Castbar functions are now in kori_castbar.lua module
...
_G.KoriUI_UpdateAnchoredUnitFrames = function()
    ...
    if playerAnchorType and playerAnchorType ~= "disabled" and KORI_UF.frames.player then
        ...
        if anchorFrame and anchorFrame:IsShown() then
            ...
            if anchor then
                local frame = KORI_UF.frames.player
    ...
    if targetAnchorType and targetAnchorType ~= "disabled" and KORI_UF.frames.target then
        local anchorFrame = GetAnchorFrame(targetAnchorType)
        if anchorFrame and anchorFrame:IsShown() then
            local anchor = GetAnchorDimensions(anchorFrame, targetAnchorType)
            if anchor then
                local frame = KORI_UF.frames.target
                local frameWidth = frame:GetWidth()
                local frameHeight = frame:GetHeight()
...
_G.KoriUI_UpdateCDMAnchoredUnitFrames = _G.KoriUI_UpdateAnchoredUnitFrames
...
_G.KoriUI_UpdateLockedCastbarToEssential = function(forceUpdate)
    local db = GetDB()
    ...
    if _G.KoriUI_RefreshCastbar then
        _G.KoriUI_RefreshCastbar("player")
    end
end

-- Global callback for NCDM to update castbar anchored to Utility
-- Width is now controlled by dual anchors, so just refresh the castbar
_G.KoriUI_UpdateLockedCastbarToUtility = function(forceUpdate)
    local db = GetDB()
    if not db or not db.player then return end
...
_G.KoriUI_UpdateLockedCastbarToFrame = function()
    local db = GetDB()
    if not db or not db.player then return end
