...
function Datatexts:Register(id, datatextDef)
    if self.registry[id] then
        print("|cffff0000KoriUI:|r Datatext '" .. id .. "' is already registered!")
        return false
    end
    
    if not datatextDef.OnEnable or type(datatextDef.OnEnable) ~= "function" then
        print("|cffff0000KoriUI:|r Datatext '" .. id .. "' missing OnEnable function!")
        return false
    end
...
function Datatexts:AttachToSlot(slotFrame, datatextID, settings)
    if not slotFrame then
        print("|cffff0000KoriUI:|r Invalid slot frame provided")
        return false
    end
    ...
    if not success then
        print("|cffff0000KoriUI:|r Failed to enable datatext '" .. datatextID .. "': " .. tostring(instance))
        return false
    end
            ...
            MenuUtil.CreateContextMenu(anchorFrame, function(_, root)
                ...
                for charKey, charData in pairs(db.global.goldData) do
                    ...
                    local btn = root:CreateButton(colorCode .. charKey .. "|r - " .. FormatGold(charMoney), function()
                        -- Confirm deletion
                        StaticPopupDialogs["KORI_GOLD_DELETE_CHAR"] = {
                        ...
                        StaticPopup_Show("KORI_GOLD_DELETE_CHAR")
                    end)
                ...
                root:CreateButton("|cffFF6666Reset All (Keep Current)|r", function()
                    StaticPopupDialogs["KORI_GOLD_RESET_ALL"] = {
                    ...
                    StaticPopup_Show("KORI_GOLD_RESET_ALL")
                end)
            end)
        end
...
-- Use shared dungeon data from kori_dungeon_data.lua
local function GetShortDungeonName(mapID)
    if _G.KORI_DungeonData then
        return _G.KORI_DungeonData.GetShortName(mapID)
    end
                            ...
                            if specName then
                                ...
                                root:CreateButton(iconText .. " " .. specName .. (isActive and activeMarker or ""), function()
                                    if InCombatLockdown() then
                                        print("|cffFF6B6BKoriUI:|r Cannot change specialization in combat")
                                        return
                                    end
