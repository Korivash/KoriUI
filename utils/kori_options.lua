--[[
    KoriUI Options Pages
    Top-down flow layout for the /kori GUI
...
---------------------------------------------------------------------------
-- KORI RECOMMENDED FPS SETTINGS (58 CVars)
---------------------------------------------------------------------------
local KORI_FPS_CVARS = {
    -- Graphics Tab
    ["vsync"] = "0",
    ["LowLatencyMode"] = "3",
    ["MSAAQuality"] = "0",
...
local function GetFontList()
    ...
    if LSM then
        ...
        for _, name in ipairs(LSM:List("font")) do
            ...
            -- Only allow fonts from WoW defaults, KoriUI, or SharedMedia
            local isWoWFont = pathLower:find("^fonts\\") ~= nil or pathLower:find("^fonts/") ~= nil
            local isKorivashFont = pathLower:find("koriui") ~= nil
            local isSharedMediaFont = pathLower:find("sharedmedia") ~= nil

            if (isWoWFont or isKorivashFont or isSharedMediaFont) and path ~= "" then
...
local function BackupCurrentFPSSettings()
    local db = GetDB()
    local backup = {}
    for cvar, _ in pairs(KORI_FPS_CVARS) do
...
local function RestorePreviousFPSSettings()
    ...
    if not db.fpsBackup then
        print("|cffFF6B6BKoriUI:|r No backup found. Apply FPS settings first to create a backup.")
        return false
    end
...
local function ApplyKorivashFPSSettings()
    ...
    for cvar, value in pairs(KORI_FPS_CVARS) do
        local success = pcall(function()
...
local function CheckCVarsMatch()
    ...
    for cvar, expectedVal in pairs(KORI_FPS_CVARS) do
...
local function RefreshUIHider()
    if _G.KoriUI_RefreshUIHider then _G.KoriUI_RefreshUIHider() end
end
...
local function CreateGeneralQoLPage(parent)
    ...
    local function RefreshCrosshair()
        if _G.KoriUI_RefreshCrosshair then
            _G.KoriUI_RefreshCrosshair()
        end
    end

    -- Refresh callback for reticle
    local function RefreshReticle()
        if _G.KoriUI_RefreshReticle then
            _G.KoriUI_RefreshReticle()
        end
    end
    ...
    local function BuildGeneralTab(tabContent)
        ...
        if db and db.general then
            ...
            local presetSummary = GUI:CreateLabel(tabContent,
                "Hover over any preset for details. 1440p+ is Korivash's personal setting.",
                11, C.textMuted)
        ...
        -- Korivash Recommended FPS Settings Section
        local fpsHeader = GUI:CreateSectionHeader(tabContent, "Korivash Recommended FPS Settings")
        fpsHeader:SetPoint("TOPLEFT", PADDING, y)
        y = y - fpsHeader.gap

        local fpsDesc = GUI:CreateLabel(tabContent,
            "Apply Korivash's optimized graphics settings for competitive play. " ..
        ...
        local previewEnterBtn = GUI:CreateButton(tabContent, "Preview +Combat", 140, 28, function()
            if _G.KoriUI_PreviewCombatText then _G.KoriUI_PreviewCombatText("+Combat") end
        end)
        ...
        if combatTextDB then
            local combatTextCheck = GUI:CreateFormCheckbox(tabContent, "Enable Combat Text", "enabled", combatTextDB, function(val)
                if _G.KoriUI_RefreshCombatText then _G.KoriUI_RefreshCombatText() end
            end)
        ...
        if combatTimerDB then
            local combatTimerCheck = GUI:CreateFormCheckbox(tabContent, "Enable Combat Timer", "enabled", combatTimerDB, function(val)
                if _G.KoriUI_RefreshCombatTimer then _G.KoriUI_RefreshCombatTimer() end
            end)
            ...
            -- Preview toggle
            local previewState = { enabled = _G.KoriUI_IsCombatTimerPreviewMode and _G.KoriUI_IsCombatTimerPreviewMode() or false }
            local previewCheck = GUI:CreateFormCheckbox(tabContent, "Preview Combat Timer", "enabled", previewState, function(val)
                if _G.KoriUI_ToggleCombatTimerPreview then
                    _G.KoriUI_ToggleCombatTimerPreview(val)
                end
            end)
        ...
        previewBtn:SetScript("OnClick", function()
            if _G.KoriUI_ToggleRaidBuffsPreview then
                local isPreview = _G.KoriUI_ToggleRaidBuffsPreview()
        ...
        local qsEnableCheck = GUI:CreateFormCheckbox(tabContent, "Enable Quick Salvage", "enabled", qsDB, function()
            if _G.KoriUI_RefreshQuickSalvage then _G.KoriUI_RefreshQuickSalvage() end
        end)
        ...
        local fontSizeSlider = GUI:CreateFormSlider(tabContent, "Key Tracker Font Size", 7, 12, 1, "keyTrackerFontSize", db.general, function()
            if _G.KoriUI_RefreshKeyTrackerFonts then
                _G.KoriUI_RefreshKeyTrackerFonts()
            end
        end)
        fontSizeSlider:SetPoint("TOPLEFT", PADDING, y)
        fontSizeSlider:SetPoint("RIGHT", tabContent, "RIGHT", -PADDING, 0)
        ...
        if minimapBtnDB then
            local showMinimapIconCheck = GUI:CreateFormCheckbox(tabContent, "Hide KORI Minimap Icon", "hide", minimapBtnDB, function(dbVal)
                ...
                if LibDBIcon then
                    if dbVal then
                        LibDBIcon:Hide("KoriUI")
                    else
                        LibDBIcon:Show("KoriUI")
                    end
                end
            end)
    ...
    local function BuildHUDVisibilityTab(tabContent)
        ...
        local function RefreshCDMVisibility()
            if _G.KoriUI_RefreshCDMVisibility then
                _G.KoriUI_RefreshCDMVisibility()
            end
        end
        ...
        local cdmMouseoverCheck = GUI:CreateFormCheckbox(tabContent, "Show On Mouseover", "showOnMouseover", cdmVis, function()
            RefreshCDMVisibility()
            if _G.KoriUI_RefreshCDMMouseover then
                _G.KoriUI_RefreshCDMMouseover()
            end
        end)
        ...
        local function RefreshUnitframesVisibility()
            if _G.KoriUI_RefreshUnitframesVisibility then
                _G.KoriUI_RefreshUnitframesVisibility()
            end
        end
        ...
        local ufMouseoverCheck = GUI:CreateFormCheckbox(tabContent, "Show On Mouseover", "showOnMouseover", ufVis, function()
            RefreshUnitframesVisibility()
            if _G.KoriUI_RefreshUnitframesMouseover then
                _G.KoriUI_RefreshUnitframesMouseover()
            end
        end)
        ...
        local function RefreshCustomTrackersVisibility()
            if _G.KoriUI_RefreshCustomTrackersVisibility then
                _G.KoriUI_RefreshCustomTrackersVisibility()
            end
        end
        ...
        local ctMouseoverCheck = GUI:CreateFormCheckbox(tabContent, "Show On Mouseover", "showOnMouseover", ctVis, function()
            RefreshCustomTrackersVisibility()
            if _G.KoriUI_RefreshCustomTrackersMouseover then
                _G.KoriUI_RefreshCustomTrackersMouseover()
            end
        end)
    ...
    local function BuildChatTab(tabContent)
        ...
        local function RefreshChat()
            if _G.KoriUI_RefreshChat then
                _G.KoriUI_RefreshChat()
            end
        end
    ...
    local function BuildTooltipTab(tabContent)
        ...
        local function RefreshTooltips()
            if _G.KoriUI_RefreshTooltips then
                _G.KoriUI_RefreshTooltips()
            end
        end
    ...
    local function BuildDragonridingTab(tabContent)
        ...
        local function RefreshSkyriding()
            if _G.KoriUI_RefreshSkyriding then
                _G.KoriUI_RefreshSkyriding()
            end
        end
    ...
    local function BuildSkinningTab(tabContent)
        ...
        if db and db.general then
            ...
            local function RefreshAllSkinning()
                if _G.KoriUI_RefreshKeystoneColors then
                    _G.KoriUI_RefreshKeystoneColors()
                end
                if _G.KoriUI_RefreshAlertColors then
                    _G.KoriUI_RefreshAlertColors()
                end
                if _G.KoriUI_RefreshLootColors then
                    _G.KoriUI_RefreshLootColors()
                end
                if _G.KoriUI_RefreshMPlusTimerColors then
                    _G.KoriUI_RefreshMPlusTimerColors()
                end
                if _G.KoriUI_RefreshCharacterFrameColors then
                    _G.KoriUI_RefreshCharacterFrameColors()
                end
                if _G.KoriUI_RefreshInspectColors then
                    _G.KoriUI_RefreshInspectColors()
                end
                if _G.KoriUI_RefreshPowerBarAltColors then
                    _G.KoriUI_RefreshPowerBarAltColors()
                end
                if _G.KoriUI_RefreshGameMenuColors then
                    _G.KoriUI_RefreshGameMenuColors()
                end
                if _G.KoriUI_RefreshOverrideActionBarColors then
                    _G.KoriUI_RefreshOverrideActionBarColors()
                end
                if _G.KoriUI_RefreshObjectiveTrackerColors then
                    _G.KoriUI_RefreshObjectiveTrackerColors()
                end
                if _G.KoriUI_RefreshInstanceFramesColors then
                    _G.KoriUI_RefreshInstanceFramesColors()
                end
                if _G.KoriUI_RefreshReadyCheckColors then
                    _G.KoriUI_RefreshReadyCheckColors()
                end
            end
            ...
            if general.addKoriUIButton == nil then general.addKoriUIButton = false end
            ...
            local gameMenuFontSlider = GUI:CreateFormSlider(tabContent, "Button Font Size", 8, 18, 1, "gameMenuFontSize", general, function()
                if _G.KoriUI_RefreshGameMenuFontSize then
                    _G.KoriUI_RefreshGameMenuFontSize()
                end
            end)
            ...
            local rcMoveBtn = GUI:CreateButton(tabContent, "Toggle Mover", 140, 28, function()
                if _G.KoriUI_ToggleReadyCheckMover then
                    _G.KoriUI_ToggleReadyCheckMover()
                end
            end)
            ...
            local powerBarMoverBtn = GUI:CreateButton(tabContent, "Toggle Position Mover", 160, 28, function()
                if _G.KoriUI_TogglePowerBarAltMover then
                    _G.KoriUI_TogglePowerBarAltMover()
                end
            end)
            ...
            local layoutDropdown = GUI:CreateFormDropdown(tabContent, "Layout Mode", layoutOptions, "layoutMode", mplusTimer, function()
                local MPlusTimer = _G.KoriUI_MPlusTimer
                if MPlusTimer and MPlusTimer.UpdateLayout then
                    MPlusTimer:UpdateLayout()
                end
                if _G.KoriUI_ApplyMPlusTimerSkin then
                    _G.KoriUI_ApplyMPlusTimerSkin()
                end
            end)
            ...
            local objTrackerHeightSlider = GUI:CreateFormSlider(tabContent, "Max Height", 200, 1000, 10,
                "objectiveTrackerHeight", general, function()
                    if _G.KoriUI_RefreshObjectiveTracker then _G.KoriUI_RefreshObjectiveTracker() end
                end)
...
local function RefreshNCDM()
    ...
    if _G.KoriUI_RefreshNCDM then
        _G.KoriUI_RefreshNCDM()
    end
end
...
local function CreateCDMSetupPage(parent)
    ...
    local function BuildUtilityTab(tabContent)
        ...
        if db and db.ncdm and db.ncdm.utility then
            ...
            local anchorCheck = GUI:CreateFormCheckbox(tabContent, "Anchor Below Essential Rows", "anchorBelowEssential", util, function()
                RefreshNCDM()
                if _G.KoriUI_ApplyUtilityAnchor then
                    _G.KoriUI_ApplyUtilityAnchor()
                end
            end)
    ...
    local function BuildBuffTab(tabContent)
        ...
        local function RefreshBuff()
            if _G.KoriUI_RefreshBuffBar then
                _G.KoriUI_RefreshBuffBar()
            end
        end
    ...
    local function BuildPowerbarTab(tabContent)
        ...
        snapSecEssentialBtn:SetScript("OnClick", function()
            if _G.KoriUI_RefreshNCDM then _G.KoriUI_RefreshNCDM() end
            local essentialViewer = _G.EssentialCooldownViewer
            if essentialViewer and essentialViewer:IsShown() then
                local rawCenterX, rawCenterY = essentialViewer:GetCenter()
                local rawScreenX, rawScreenY = UIParent:GetCenter()
...
local function CreateCDEffectsPage(parent)
    ...
    local function RefreshSwipe()
        if _G.KoriUI_RefreshCooldownSwipe then _G.KoriUI_RefreshCooldownSwipe() end
    end
    local function RefreshEffects()
        if _G.KoriUI_RefreshCooldownEffects then _G.KoriUI_RefreshCooldownEffects() end
    end
    local function RefreshGlows()
        if _G.KoriUI_RefreshCustomGlows then _G.KoriUI_RefreshCustomGlows() end
    end
    ...
    local swipeDesc = GUI:CreateLabel(content, "Control which animations appear on CDM icons. Korivash's personal setup is to turn OFF all the below.", 11, C.textMuted)
...
local function CreateCDKeybindsPage(parent)
    ...
    local function RefreshKeybinds()
        if _G.KoriUI_RefreshKeybinds then
            _G.KoriUI_RefreshKeybinds()
        end
    end

    -- Refresh function for rotation helper
    local function RefreshRotationHelper()
        if _G.KoriUI_RefreshRotationHelper then
            _G.KoriUI_RefreshRotationHelper()
        end
    end
    ...
    if db and db.viewers then
        ...
        local function RefreshCustomTrackerKeybinds()
            if _G.KoriUI_RefreshCustomTrackerKeybinds then
                _G.KoriUI_RefreshCustomTrackerKeybinds()
            end
        end
        ...
        local function RefreshRAI()
            if _G.KoriUI_RefreshRotationAssistIcon then
                _G.KoriUI_RefreshRotationAssistIcon()
            end
        end
...
local function CreateCustomTrackersPage(parent)
    ...
    ---------------------------------------------------------------------------
    -- Helper: Calculate offset relative to KORI_Player frame's top-left corner
...
local function CreateUnitFramesPage(parent)
    ...
    local function BuildGeneralTab(tabContent)
        ...
        local editDesc = GUI:CreateLabel(tabContent, "Toggle Edit Mode to drag and reposition unit frames. Or use /kori editmode", 11, C.textMuted)
    ...
    local function BuildUnitTab(tabContent, unitKey)
        ...
        local function RefreshAuras()
            ...
            local KORI_UF = ns.KORI_UnitFrames
            if KORI_UF and KORI_UF.auraPreviewMode then
                if KORI_UF.auraPreviewMode[unitKey .. "_debuff"] then
                    _G.KoriUI_ShowAuraPreview(unitKey, "debuff")
                end
                if KORI_UF.auraPreviewMode[unitKey .. "_buff"] then
                    _G.KoriUI_ShowAuraPreview(unitKey, "buff")
                end
            end
            -- Refresh real auras if not in preview mode
            if _G.KoriUI_RefreshAuras then
                _G.KoriUI_RefreshAuras(unitKey)
            end
        end
        ...
        previewTrack:SetScript("OnClick", function()
            ...
            UpdatePreviewToggle(isPreviewOn)
            if isPreviewOn then
                if _G.KoriUI_ShowUnitFramePreview then _G.KoriUI_ShowUnitFramePreview(unitKey) end
            else
                if _G.KoriUI_HideUnitFramePreview then _G.KoriUI_HideUnitFramePreview(unitKey) end
            end
        end)
        y = y - FORM_ROW
        ...
        if unitKey == "player" then
            widthCallback = function()
                RefreshUnit()
                if _G.KoriUI_UpdateLockedCastbarToFrame then
                    _G.KoriUI_UpdateLockedCastbarToFrame()
                end
            end
        end
        ...
        -- Register sliders for real-time sync during Edit Mode
        if _G.KoriUI_RegisterEditModeSliders then
            _G.KoriUI_RegisterEditModeSliders(unitKey, offsetXSlider, offsetYSlider)
        end
        ...
        if unitKey == "player" or unitKey == "target" then
            ...
            local anchorDropdown = GUI:CreateFormDropdown(tabContent, "Anchor To", anchorOptions, "anchorTo", unitDB, function()
                RefreshUnit()
                if _G.KoriUI_UpdateAnchoredUnitFrames then
                    _G.KoriUI_UpdateAnchoredUnitFrames()
                end
        ...
        if unitKey == "player" or unitKey == "target" or unitKey == "targettarget" or unitKey == "focus" or unitKey == "pet" or unitKey == "boss" then
            ...
            if ns.KORI_CastbarOptions and ns.KORI_CastbarOptions.BuildCastbarOptions then
                y = ns.KORI_CastbarOptions.BuildCastbarOptions(tabContent, unitKey, y, PAD, FORM_ROW, RefreshUnit, GetTextureList, NINE_POINT_ANCHOR_OPTIONS, GetUFDB, GetDB)
            end
        end
           ...
           or unitKey == "pet" or unitKey == "targettarget" or unitKey == "boss" then
            ...
            debuffPreviewTrack:SetScript("OnClick", function()
                ...
                if isDebuffPreviewOn then
                    if _G.KoriUI_ShowAuraPreview then
                        _G.KoriUI_ShowAuraPreview(unitKey, "debuff")
                    end
                else
                    if _G.KoriUI_HideAuraPreview then
                        _G.KoriUI_HideAuraPreview(unitKey, "debuff")
                    end
                end
            end)
            y = y - FORM_ROW
            ...
            buffPreviewTrack:SetScript("OnClick", function()
                ...
                if isBuffPreviewOn then
                    ...
                    if _G.KoriUI_HideAuraPreview then
                        _G.KoriUI_HideAuraPreview(unitKey, "buff")
                    end
                end
            end)
            y = y - FORM_ROW
...
local function CreateActionBarsPage(parent)
    ...
    local function RefreshActionBars()
        if _G.KoriUI_RefreshActionBars then
            _G.KoriUI_RefreshActionBars()
        end
    end
        ...
        local function BuildBarSettingsIntoContainer(barKey, container, onOverrideChanged)
            ...
            if barKey == "bar1" then
                local pageArrowToggle = GUI:CreateFormCheckbox(container,
                    ...
                    function(val)
                        if _G.KoriUI_ApplyPageArrowVisibility then
                            _G.KoriUI_ApplyPageArrowVisibility(val)
                        end
                    end)
    ...
    local function BuildExtraButtonsTab(tabContent)
        ...
        local function RefreshExtraButtons()
            if _G.KoriUI_RefreshExtraButtons then
                _G.KoriUI_RefreshExtraButtons()
            end
        end
        ...
        -- Toggle Movers Button
        local moverBtn = GUI:CreateButton(tabContent, "Toggle Position Movers", 200, 28, function()
            if _G.KoriUI_ToggleExtraButtonMovers then
                _G.KoriUI_ToggleExtraButtonMovers()
            end
        end)
        moverBtn:SetPoint("TOPLEFT", PAD, y)
        y = y - 35
...
---------------------------------------------------------------------------
-- SUB-TAB BUILDER: Korivash's Strings (preset import strings)
---------------------------------------------------------------------------
local function BuildKorivashStringsTab(tabContent)
    local y = -10
    local PAD = 10
    local BOX_HEIGHT = 70
    ...
    -- EDIT MODE STRING
    -- =====================================================
    local editModeHeader = GUI:CreateSectionHeader(tabContent, "Korivash Edit Mode String")
    editModeHeader:SetPoint("TOPLEFT", PAD, y)
    y = y - editModeHeader.gap

    local editModeString = ""
    if _G.KoriUI and _G.KoriUI.imports and _G.KoriUI.imports.EditMode then
        editModeString = _G.KoriUI.imports.EditMode.data or ""
    end
    ...
    local platString = ""
    if _G.KoriUI and _G.KoriUI.imports and _G.KoriUI.imports.Platynator then
        platString = _G.KoriUI.imports.Platynator.data or ""
    end
...
local function CreateImportExportPage(parent)
    ...
    local subTabs = GUI:CreateSubTabs(content, {
        ...
        {name = "Korivash's Strings", builder = BuildKorivashStringsTab},
    })
    subTabs:SetPoint("TOPLEFT", 5, -5)
    subTabs:SetPoint("TOPRIGHT", -5, -5)
...
local function CreateSpecProfilesPage(parent)
    ...
    _G.KoriUI_RefreshSpecProfilesTab = RefreshProfileDisplay
    ...
    local deleteDropdown = GUI:CreateFormDropdown(content, "Delete Profile", GetProfileList(), "selected", deleteWrapper, function(value)
        if db and value and value ~= "" then
            ...
            if value == current then
                print("|cffff0000KoriUI:|r Cannot delete the active profile!")
...
local function CreateHUDLayeringPage(parent)
    ...
    local function RefreshCastbars()
        if _G.KoriUI_RefreshCastbars then
            _G.KoriUI_RefreshCastbars()
        end
    end

    local function RefreshCustomTrackers()
        if _G.KoriUI_RefreshCustomTrackers then
            _G.KoriUI_RefreshCustomTrackers()
        end
    end
