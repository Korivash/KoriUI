...
local function ApplyHideSettings()
    ...
    if GameTimeFrame then
        ...
        if not GameTimeFrame._KORI_ShowHooked then
            GameTimeFrame._KORI_ShowHooked = true
    ...
    if CompactRaidFrameManager then
        ...
        elseif settings.hideRaidFrameManager then
            ...
            if not CompactRaidFrameManager._KORI_ShowHooked then
                CompactRaidFrameManager._KORI_ShowHooked = true
            ...
            -- BUG-008: Wrap in C_Timer.After(0) to break taint chain from secure Blizzard code
            if not CompactRaidFrameManager._KORI_SetShownHooked then
                CompactRaidFrameManager._KORI_SetShownHooked = true
    ...
    if BuffFrame and BuffFrame.CollapseAndExpandButton then
        ...
        if settings.hideBuffCollapseButton then
            ...
            if not btn._KORI_AlphaHooked then
                btn._KORI_AlphaHooked = true
    ...
    if TalkingHeadFrame then
        ...
        if settings.hideTalkingHead then
            ...
            if not TalkingHeadFrame._KORI_ShowHooked then
                TalkingHeadFrame._KORI_ShowHooked = true
            ...
            if not TalkingHeadFrame._KORI_MouseManaged then
                TalkingHeadFrame._KORI_MouseManaged = true
        ...
        if not TalkingHeadFrame._KORI_MuteHooked then
            TalkingHeadFrame._KORI_MuteHooked = true
            hooksecurefunc(TalkingHeadFrame, "PlayCurrent", function()
                local s = GetSettings()
    ...
    if StatusTrackingBarManager then
        ...
        if hideXP and hideRep then
            ...
            if not StatusTrackingBarManager._KORI_ShowHooked then
                StatusTrackingBarManager._KORI_ShowHooked = true
        ...
        elseif hideXP or hideRep then
            ...
            if not StatusTrackingBarManager._KORI_BarsHooked then
                StatusTrackingBarManager._KORI_BarsHooked = true
...
-- Initialize
-- Note: ApplyHideSettings is now called from koricore_main.lua OnEnable()
