...
local function ApplyIconStyle(icon, settings)
    ...
    -- Apply stack text size using same approach as koricore_main.lua
    local fs = nil
...
local function ApplyBarStyle(frame, settings)
    ...
    if statusBar and statusBar.SetStatusBarTexture then
        local texturePath = LSM:Fetch("statusbar", texture) or LSM:Fetch("statusbar", "Korivash v5")
        if texturePath then
...
KORI_BuffBar.LayoutIcons = LayoutBuffIcons
KORI_BuffBar.LayoutBars = LayoutBuffBars
KORI_BuffBar.Initialize = Initialize

-- Force refresh function (can be called from GUI)
function KORI_BuffBar.Refresh()
...
_G.KoriUI_RefreshBuffBar = KORI_BuffBar.Refresh
