...
---------------------------------------------------------------------------
-- DATABASE ACCESS
-- Uses KoriUI.db.global.spellScanner for cross-character persistence
---------------------------------------------------------------------------
    ...
    if bestMatch then
        ...
        if success then
            ...
            if SpellScanner.scanMode then
                print(string.format("|cff00ff00KoriUI:|r Scanned: %s = %.1fs",
...
SlashCmdList["KORISCANNED"] = function()
    ...
    print("|cff00ff00KoriUI Scanned Spells:|r")
    local spellCount = 0
    ...
    print("|cff00ff00KoriUI Scanned Items:|r")
    local itemCount = 0
...
SlashCmdList["KORICLEARSPELL"] = function(msg)
    ...
    if db and db.spells and db.spells[spellID] then
        ...
        print(string.format("|cff00ff00KoriUI:|r Cleared spell: %s [%d]", name, spellID))
    else
        print(string.format("|cffff8800KoriUI:|r Spell %d not found in scanned list", spellID))
    end
end
