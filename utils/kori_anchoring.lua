...
-- SETUP HELPERS
---------------------------------------------------------------------------
function KORI_Anchoring:SetHelpers(helpers)
    Helpers = helpers or {}
end
...
function KORI_Anchoring:RegisterAnchorTarget(name, frame, options)
...
function KORI_Anchoring:UnregisterAnchorTarget(name)
...
function KORI_Anchoring:GetAnchorTarget(name)
...
function KORI_Anchoring:GetAnchorTargetList(include, exclude, excludeSelf)
    include = include or {}
    exclude = exclude or {}
...
function KORI_Anchoring:GetAnchorDimensions(anchorFrame, anchorTargetName)
...
function KORI_Anchoring:PositionFrame(frame, anchorTarget, anchorPoint, offsetX, offsetY, parentFrame, options)
...
function KORI_Anchoring:GetAnchorTargetName(frame)
...
function KORI_Anchoring:CheckCircularDependency(frame, anchorTarget)
...
function KORI_Anchoring:RegisterAnchoredFrame(frame, config)
...
function KORI_Anchoring:UnregisterAnchoredFrame(frame)
    if not frame then return false end
...
function KORI_Anchoring:SnapTo(frame, anchorTarget, anchorPoint, offsetX, offsetY, options)
...
function KORI_Anchoring:UpdateAllAnchoredFrames()
    if InCombatLockdown() then 
...
function KORI_Anchoring:UpdateFramesForTarget(anchorTargetName)
    if InCombatLockdown() then 
        -- Defer update after combat
        C_Timer.After(0, function()
...
_G.KoriUI_UpdateAnchoredFrames = function()
    if KORI_Anchoring then
        KORI_Anchoring:UpdateAllAnchoredFrames()
    end
end

-- Backward compatibility alias
_G.KoriUI_UpdateAnchoredUnitFrames = _G.KoriUI_UpdateAnchoredFrames
_G.KoriUI_UpdateCDMAnchoredUnitFrames = _G.KoriUI_UpdateAnchoredFrames
