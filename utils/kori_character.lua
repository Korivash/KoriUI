...
-- Color palette (KoriUI brand colors)
local C = {
...
local function IsSkinningHandlingBackground()
    local skinningAPI = _G.KORI_CharacterFrameSkinning
...
---------------------------------------------------------------------------
-- Create custom KoriUI background
---------------------------------------------------------------------------
local function CreateCustomBackground()
    ...
    if skinningAPI and skinningAPI.IsEnabled and skinningAPI.IsEnabled() then
        ...
        if not customBg then
            customBg = CreateFrame("Frame", "KORI_CharacterFrameBg_CharPane", CharacterFrame, "BackdropTemplate")
...
_G.KoriUI_RefreshCharacterPanelFonts = RefreshCharacterPanelFonts
...
local function UpdateStatsPanel(panel, unit)
    ...
    if not success then
        print("KoriUI: Error updating stats panel:", err)
    end
end
...
local function CreateEquipMgrPopup()
    ...
    local PANEL_WIDTH_EXTENSION = 55
    equipMgrPopup = CreateFrame("Frame", "KoriUI_EquipMgrPopup", UIParent, "BackdropTemplate")
    ...
    _G.KoriUI_EquipMgrPopup = equipMgrPopup
...
local function CreateTitlesPopup()
    ...
    -- Position accounts for extended character pane (55px width extension + 10px gap)
    local PANEL_WIDTH_EXTENSION = 55
    titlesPopup = CreateFrame("Frame", "KoriUI_TitlesPopup", UIParent, "BackdropTemplate")
    ...
    -- Expose globally for skinning module to access
    _G.KoriUI_TitlesPopup = titlesPopup
...
local function HookCharacterFrame()
    ...
    if not CharacterFrame._quiGearBtn then
        gearBtn = CreateFrame("Button", "KORI_CharacterSettingsBtn", CharacterFrame, "BackdropTemplate")
        ...
        settingsPanel = CreateFrame("Frame", "KoriUI_CharSettingsPanel", CharacterFrame, "BackdropTemplate")
        ...
        local function RefreshAll()
            if _G.KoriUI_RefreshCharacterPanelFonts then
                _G.KoriUI_RefreshCharacterPanelFonts()
            end
            ScheduleUpdate()
        end
...
eventFrame:SetScript("OnEvent", function(self, event, arg1)
    if event == "ADDON_LOADED" then
        -- Note: Blizzard_InspectUI is now hooked by kori_inspect.lua
...
---------------------------------------------------------------------------
_G.KoriUI_RefreshCharacterPane = function()
    ScheduleUpdate()
end
