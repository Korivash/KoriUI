...
local function AddBorderToButton(button, isBuff)
    ...
    if not button.koriBorderTop then
        -- Top border
        button.koriBorderTop = button:CreateTexture(nil, "OVERLAY", nil, 7)
        button.koriBorderTop:SetPoint("TOPLEFT", icon, "TOPLEFT", 0, 0)
        button.koriBorderTop:SetPoint("TOPRIGHT", icon, "TOPRIGHT", 0, 0)
        
        -- Bottom border
        button.koriBorderBottom = button:CreateTexture(nil, "OVERLAY", nil, 7)
        button.koriBorderBottom:SetPoint("BOTTOMLEFT", icon, "BOTTOMLEFT", 0, 0)
        button.koriBorderBottom:SetPoint("BOTTOMRIGHT", icon, "BOTTOMRIGHT", 0, 0)
        
        -- Left border
        button.koriBorderLeft = button:CreateTexture(nil, "OVERLAY", nil, 7)
        button.koriBorderLeft:SetPoint("TOPLEFT", icon, "TOPLEFT", 0, 0)
        button.koriBorderLeft:SetPoint("BOTTOMLEFT", icon, "BOTTOMLEFT", 0, 0)
        
        -- Right border
        button.koriBorderRight = button:CreateTexture(nil, "OVERLAY", nil, 7)
        button.koriBorderRight:SetPoint("TOPRIGHT", icon, "TOPRIGHT", 0, 0)
        button.koriBorderRight:SetPoint("BOTTOMRIGHT", icon, "BOTTOMRIGHT", 0, 0)
    end
    
    -- Update border color based on type
    button.koriBorderTop:SetColorTexture(borderColor[1], borderColor[2], borderColor[3], borderColor[4])
    button.koriBorderBottom:SetColorTexture(borderColor[1], borderColor[2], borderColor[3], borderColor[4])
    button.koriBorderLeft:SetColorTexture(borderColor[1], borderColor[2], borderColor[3], borderColor[4])
    button.koriBorderRight:SetColorTexture(borderColor[1], borderColor[2], borderColor[3], borderColor[4])
    
    -- Update border size
    button.koriBorderTop:SetHeight(borderSize)
    button.koriBorderBottom:SetHeight(borderSize)
    button.koriBorderLeft:SetWidth(borderSize)
    button.koriBorderRight:SetWidth(borderSize)
    
    button.koriBorderTop:Show()
    button.koriBorderBottom:Show()
    button.koriBorderLeft:Show()
    button.koriBorderRight:Show()
...
local function HideBorderOnButton(button)
    if button.koriBorderTop then button.koriBorderTop:Hide() end
    if button.koriBorderBottom then button.koriBorderBottom:Hide() end
    if button.koriBorderLeft then button.koriBorderLeft:Hide() end
    if button.koriBorderRight then button.koriBorderRight:Hide() end
end
...
local function ApplyFrameHiding()
    ...
    if BuffFrame then
        ...
        if not BuffFrame._KORI_ShowHooked then
            BuffFrame._KORI_ShowHooked = true
    ...
    if DebuffFrame then
        ...
        -- Hook Show() once to prevent Blizzard from re-showing
        if not DebuffFrame._KORI_ShowHooked then
            DebuffFrame._KORI_ShowHooked = true
...
-- Note: Initial application is now called from koricore_main.lua OnEnable() to ensure AceDB is ready
...
_G.KoriUI_RefreshBuffBorders = function()
