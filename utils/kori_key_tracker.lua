...
local function GetDungeonInfo(mapID)
    ...
    if _G.KORI_DungeonData then
        local data = _G.KORI_DungeonData.GetDungeonData(mapID)
        if data then
...
local function GetKeyColor(level)
    ...
    if _G.KORI_DungeonData and _G.KORI_DungeonData.GetKeyColor then
        return _G.KORI_DungeonData.GetKeyColor(level)
    end
    -- Fallback
...
_G.KoriUI_RefreshKeyTrackerColors = ApplySkinColors
...
_G.KoriUI_RefreshKeyTrackerFonts = UpdateAllButtonFonts
