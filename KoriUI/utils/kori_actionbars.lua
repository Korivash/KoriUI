...
local TEXTURE_PATH = [[Interface\AddOns\KoriUI\assets\iconskin\]]
local TEXTURES = {
...
local function CreateExtraButtonHolder(buttonType, displayName)
    ...
    -- Create holder frame
    local holder = CreateFrame("Frame", "KORI_" .. buttonType .. "Holder", UIParent)
    ...
    local mover = CreateFrame("Frame", "KORI_" .. buttonType .. "Mover", holder, "BackdropTemplate")
...
_G.KoriUI_ToggleExtraButtonMovers = ToggleExtraButtonMovers
_G.KoriUI_RefreshExtraButtons = RefreshExtraButtons
...
local function ApplyPageArrowVisibility(hide)
    ...
    if hide then
        pageNum:Hide()
        if not pageNum._KORI_ShowHooked then
            pageNum._KORI_ShowHooked = true
...
_G.KoriUI_ApplyPageArrowVisibility = ApplyPageArrowVisibility
...
_G.KoriUI_RefreshActionBars = function()
    if InCombatLockdown() then
