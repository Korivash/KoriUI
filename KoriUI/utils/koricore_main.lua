...
function KORICore:SafeReload()
    if InCombatLockdown() then
        if not self.__pendingReload then
            ...
            print("|cFF1E90FFKori UI:|r Reload queued - will execute when combat ends.")
...
function KORICore:ShowReloadPopup()
    ...
    if KoriUI and KoriUI.GUI and KoriUI.GUI.ShowConfirmation then
        ...
        print("|cFF1E90FFKori UI:|r Combat ended. Type /reload to reload.")
    end
end
...
local defaults = {
    profile = {
        ...
        general = {
            ...
            font = "Korivash",  -- Default font face
            fontOutline = "OUTLINE",  -- Default font outline: "", "OUTLINE", "THICKOUTLINE"
            texture = "Korivash v5",  -- Default bar texture
            darkMode = false,
            ...
            addKoriUIButton = false,  -- Add Kori UI button to ESC menu (opt-in)
        ...
        combatTimer = {
            ...
            font = "Korivash",       -- Font name (from LibSharedMedia)
...
function KORICore:OnProfileChanged(event, db, profileKey)
    ...
    if _G.KoriUI_RefreshSpecProfilesTab then
        _G.KoriUI_RefreshSpecProfilesTab()
    end
...
function KORICore:HideCurrentSelectionArrows()
    ...
    if sel.selectedType == "unitframe" then
        ...
        if ns.KORI_UnitFrames and ns.KORI_UnitFrames.frames then
            local frame = ns.KORI_UnitFrames.frames[sel.selectedKey]
...
function KORICore:OnEnable()
    ...
    C_Timer.After(1.0, function()
        ...
        if _G.KoriUI_RefreshUIHider then
...
function KORICore:CreateMinimapButton()
    ...
    local dataObj = LDB:NewDataObject(ADDON_NAME, {
        type = "launcher",
        icon = "Interface\\AddOns\\KoriUI\\assets\\koriLogo.tga",
        label = "Kori UI",
        ...
        OnTooltipShow = function(tooltip)
            tooltip:SetText("|cFF1E90FFKori UI|r")
...
-- Fallback to bundled Korivash font (always available, loaded early in media.lua)
local KORI_FONT_PATH = [[Interface\AddOns\KoriUI\assets\Korivash.ttf]]
